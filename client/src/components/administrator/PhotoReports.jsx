...
const handleStatusUpdate = (productId, reportId, newStatus) => {
  axiosClient
    .put(`/products/reports/${productId}/updateStatus/${reportId}`, {
      ...
      <div className="w-full min-h-[calc(100vh-65px)] max-h-[calc(100vh-65px)] overflow-y-scroll p-4 bg-gray-200">
        {!allProducts ? (
          ...
          allProducts
            .filter(
              (product) =>
                Array.isArray(product.reportedIssues) &&
                product.reportedIssues.length > 0
            )
            .map((product) => (
              <div
                key={product.id || product.productNumber}
                className={`collapse bg-slate-100 collapse border border-slate-300 rounded-md mb-2 ${
                  openId === product.id ? "collapse-open" : ""
                }`}
                onClick={() =>
              ...
              >
                <div className="flex justify-between collapse-title text-gray-700 font-semibold hover:bg-[rgb(252,217,185)] transition-colors px-4 py-2">
                  ...
                  <p>{product.reportedIssues.length} report(s)</p>
                </div>

                <div className="collapse-content text-slate-600 px-4 flex flex-col gap-2">
                  {product.reportedIssues.map((report, idx) => (
                    <div
                      key={`${
                        ...
                        <div className="flex flex-col items-center justify-center pr-8">
                          ...
                          <select
                            ...
                            value={report.currentStatus}
                              onChange={(e) =>
    handleStatusUpdate(product._id, report._id, e.target.value)
  }
                          >
